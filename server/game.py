...
class Game:
    ...
    def transition(self, state, action):
        #print(f'state before transition {statePlusParamHashKey(state,self.scenarioPo)} units {state["units"]}')  
    ...
    def _transition_setup(self, state, action, unitData):
        if not self._is_legal_setup(state, action):
            # print(f'setup phase, action {action} legal actions:{ self.legal_actions(state)}')
...
def statePlusParamHashKey(state, param):
    ...
    def hexStr(x,y):
        ...
        terrainCodes = {"clear":"c", "urban":"u","rough":"r","water":"w", "coast":"t"}
